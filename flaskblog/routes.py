...
from flaskblog import app, db, bcrypt, mail # mail=Mail(app) in init.py
...
@app.route('/home')
def home():
    page= request.args.get('page',1, type=int) #default is 1 so 5 posts at one
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('Your post has been craeted!', 'success')
        return redirect(url_for('home'))
    return render_template('create_post.html',title='New Post', form=form, legend='New Post')

@app.route('/post/<int:post_id>')
def post(post_id):
    post=Post.query.get_or_404(post_id) #if post by given id exists then return it in a template or return a 404
...
def user_posts(username):
    ...
    return render_template('user_posts.html',posts=posts, user=user) #posts=posts tell that, we can use 'posts' variale (which is defined here) as 'posts' in any templates

def send_reset_email(user):
    token = user.get_reset_token()
    msg=Message('Password reset request', sender='silenme3@gmail.com' , recipients=[user.email])
...
def reset_token(token):   
    ...
    if user is None:
        flash('Invalid Token or session timeout', 'warning')
